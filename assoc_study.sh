...
output_values_of_PCA=${outdirectory}${todays_date}_PCA/ #to be used as covariate file

#We will check whether the PCA script was run, if it was then we will use the output of the PCA as covar file
if [ -d "$output_values_of_PCA" ]; then
    ...
    echo "PCA script was not run, input files for covariates must be in extrafiles in the following path: ${path_to_extrafiles}"
...
#We will also use a manhattan plot to watch the results in a graphical way
...
#2.file of association results only where the values are the variant results (not the PCA assoc results), to check this the flag for plink --hide-covar must be used
...
#with their MAF count (obtained from the general QC script for the association study) and the variant info obtained from the VCF file

#Define output variabes
...
#Next variables are used within the python script to save some intermediate steps
#4. path to the output file which is a csv file
#5. path to the second output file with the critical p values

#Finally we will save the top 10 most significant snps in a separate file
